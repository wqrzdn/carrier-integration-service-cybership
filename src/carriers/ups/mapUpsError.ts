...
/*
 * This takes any messy error-whether 
 ...
 * 
 * Enhanced with CarrierErrorCode enum for consistent error classification.
 */
  ...
  if (err instanceof HttpError) {
    if (err.status === 401) {
      return new CarrierError(
        ...
        'UPS',
        true,
        'UPS returned 401 - auth token may have been invalidated',
        err,
        err.status
      );
    }
    ...
    if (err.status === 429) {
      return new CarrierError(
        ...
        'UPS',
        true,
        'UPS rate limit exceeded - back off before retrying',
        err,
        err.status
      );
    }
    ...
    if (err.status === 502 || err.status === 503 || err.status === 504) {
      return new CarrierError(
        CarrierErrorCode.UPSTREAM_UNAVAILABLE,
        'UPS',
        true,
        `UPS service temporarily unavailable (HTTP ${err.status})`,
        err,
        err.status
      );
    }
    ...
    if (err.status >= 500) {
      return new CarrierError(
        ...
        'UPS',
        true,
        `UPS server error (HTTP ${err.status})`,
        err,
        err.status
      );
    }
    ...
    return new CarrierError(
      CarrierErrorCode.INVALID_REQUEST,
      'UPS',
      false,
      `UPS rejected the request (HTTP ${err.status})`,
      err,
      err.status
    );
  }
  ...
  if (err instanceof Error) {
    ...
    if (err instanceof ZodError) {
      return new CarrierError(
        ...
        'UPS',
        false,
        'UPS response validation failed',
        err
      );
    }
    ...
    return new CarrierError(
      ...
      'UPS',
      false,
      `UPS response was malformed: ${err.message}`,
      err
    );
  }
